...
class PreferenceManager: NSObject {
	...
	var followingArtists: [Artist] {
		...
		get {
			return self._followingArtists.sorted(by: { (first, second) -> Bool in
				var firstname = first.name.lowercased()
				var secondname = second.name.lowercased()
				if firstname.hasPrefix("the ") {
					firstname = firstname.substring(from: firstname.index(firstname.startIndex, offsetBy: 4))
				}
				if secondname.hasPrefix("the ") {
					secondname = secondname.substring(from: secondname.index(secondname.startIndex, offsetBy: 4))
				}
	...
	func follow(artist: Artist) {
		if !artist.isBeingFollowed {
			self.followingArtists.append(artist)
			
			// trigger artwork cache
			self.cacheArtwork(artist: artist)
			
			// get releases for the newly added artist
			self.updateNewReleases(for: [artist], force: true)
		}
	...
	func unfollow(artist: Artist) {
		...
		// trigger artwork cache removal
		artist.releases.forEach({ self.uncacheArtwork(release: $0 )})
		self.uncacheArtwork(artist: artist)
						...
						self.nowPlayingArtistLookupTask = RequestManager.shared.search(for: artistName, completion: { (artists, error) in
							...
							if !artist.isBeingFollowed {
								...
								self.nowPlayingArtistLookupTask = RequestManager.shared.getAdditionalInfo(for: artist, completion: { (artistWithInfo, error) in
									...
									print("Getting releases for now playing artist...")
									self.nowPlayingArtistLookupTask = RequestManager.shared.getReleases(for: artistWithInfo, completion: { (releases, error) in
										guard let releases = releases, error == nil else {
											print("Couldn't get releases for the currently playing artist: ", error!)
											self.nowPlayingArtist = artistWithInfo
											return
										}
										
										artistWithInfo.releases = releases
										
										print("Loading artwork for now playing artist...")
	...
	override init() {
		...
		if !FileManager.default.fileExists(atPath: self.artworkCachePath.absoluteString) {
			try? FileManager.default.createDirectory(at: self.artworkCachePath, withIntermediateDirectories: true, attributes: nil)
		}
	...
	func load(completion: (() -> Void)?=nil) {
		...
		DispatchQueue.global().async {
			...
			if let encodedFollowingArtists = UserDefaults.standard.object(forKey: self.followingArtistsKey) as? Data,
				...
				// BIG ISSUE - Following Artists are corrupted.
				self.followingArtists = []
			}
	...
	private let artworkCachePath = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0].appendingPathComponent("artwork")
	
	let thumbnailSize = CGSize(width: 64, height: 64)
	let artworkSize = CGSize(width: max(UIScreen.main.bounds.width, UIScreen.main.bounds.height), height: max(UIScreen.main.bounds.width, UIScreen.main.bounds.height))
	
	func cacheArtwork(artist: Artist, force: Bool?=false) {
		guard let image = artist.artworkImage else {
			return
		}
		...
		let filepath = self.artworkCachePath.appendingPathComponent("artist_\(artist.itunesID!).jpg")
		if FileManager.default.fileExists(atPath: filepath.path) && !force! {
			print("Artwork for \(artist.name!) already stored. No need to save.")
			return
		}
		
		FileManager.default.createFile(atPath: filepath.path, contents: data, attributes: nil)
		
		print("Saved artwork for \(artist.name!) to filesystem.")
	}
	func loadArtwork(artist: Artist) -> UIImage? {
		
		let filepath = self.artworkCachePath.appendingPathComponent("artist_\(artist.itunesID!).jpg")
		if !FileManager.default.fileExists(atPath: filepath.path) {
			print("Artwork for \(artist.name!) not found in filesystem.")
			return nil
		}
		
		// file was found
		print("Found artwork for \(artist.name!)")
		return UIImage(contentsOfFile: filepath.path)
	}
	func uncacheArtwork(artist: Artist) {
		let filepath = self.artworkCachePath.appendingPathComponent("artist_\(artist.itunesID!).jpg")
		try? FileManager.default.removeItem(at: filepath)
	}
	func cacheArtwork(release: Release, force: Bool?=false) {
		guard let image = release.artworkImage else {
			return
		}
		guard let data = UIImageJPEGRepresentation(image, 0.4) else {
			return
		...
		let filepath = self.artworkCachePath.appendingPathComponent("release_\(release.itunesID!).jpg")
		if FileManager.default.fileExists(atPath: filepath.path) && !force! {
			print("Artwork for \(release.title!) already stored. No need to save.")
			return
		}
		
		FileManager.default.createFile(atPath: filepath.path, contents: data, attributes: nil)
		
		print("Saved artwork for \(release.title!) to filesystem.")
	}
	func loadArtwork(release: Release) -> UIImage? {
		
		let filepath = self.artworkCachePath.appendingPathComponent("release_\(release.itunesID!).jpg")
		if !FileManager.default.fileExists(atPath: filepath.path) {
			print("Artwork for \(release.title!) not found in filesystem.")
			return nil
		}
		
		// file was found
		print("Found artwork for \(release.title!)")
		return UIImage(contentsOfFile: filepath.path)
	}
	func uncacheArtwork(release: Release) {
		let filepath = self.artworkCachePath.appendingPathComponent("release_\(release.itunesID!).jpg")
		try? FileManager.default.removeItem(at: filepath)
	}
