...
class ArtistSearchResultsViewController: DroppChildViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		...
		if segue.identifier == "showArtist" {
			(segue.destination as? ArtistViewController)?.currentArtist = self.selectedArtist!
		}
...
extension ArtistSearchResultsViewController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
	...
	func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
		...
		self.selectedArtist = PreferenceManager.shared.followingArtists.first(where: { $0.itunesID == self.artistSearchResults[indexPath.row].itunesID }) ?? self.artistSearchResults[indexPath.row]
		...
		RequestManager.shared.getAdditionalInfo(for: self.selectedArtist!, completion: { (artist, error) in
			...
			self.selectedArtist?.summary = artist.summary
			self.selectedArtist?.artworkURL = artist.artworkURL
			
			// load artwork - 2
			_ = self.selectedArtist?.loadArtwork {
		...
		// load releases - 3
		RequestManager.shared.getReleases(for: self.selectedArtist!, completion: { (releases, error) in
			...
			self.selectedArtist?.releases = releases
